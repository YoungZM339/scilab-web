...
- [ ] `BETTER_AUTH_URL` 与 `SITE_URL` 均为最终 HTTPS 域名，且没有尾部路径。
- [ ] 反向代理已启用有效证书，应用端口只监听 `127.0.0.1`。
- [ ] 已创建管理员并使用长度不少于 14 位的独立密码完成登录。
- [ ] 网站名称、联系方式、页脚、网页标题和网页简介已替换为真实内容。
- [ ] 首页、团队、研究、成果、动态、加入我们和联系页面在手机上检查无误。
- [ ] 草稿不能被未登录访客访问，PDF 和图片的打开、下载行为符合预期。
- [ ] `./ops/backup.sh` 已成功执行，备份已复制到另一台设备或加密远端。
- [ ] 已实际完成一次恢复演练，并记录负责人、恢复耗时和备份位置。
- [ ] 已为磁盘空间、容器健康状态和备份失败设置监控或固定巡检。
