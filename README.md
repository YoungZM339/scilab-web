...
> **项目定位**：不是通用建站平台，而是一套开箱即用、维护成本低的实验室官网。对外专注内容展示，对内提供清晰的中文管理流程。
...
| 需求                 | SciLab Web 的做法                                                  |
| -------------------- | ------------------------------------------------------------------ |
| 老师和学生都能维护   | 中文后台集中管理主页、成员、研究、成果和动态，不需要修改代码。     |
| 小团队也能承担运维   | 一个容器、两个持久卷；提供启动、备份、恢复和状态检查脚本。         |
| 内容发布可控         | 草稿、发布、重点展示和排序互相独立，首页只展示管理员选定的内容。   |
| 不希望被平台绑定     | MIT 许可，数据保存在自己的 SQLite 与上传目录中，可完整备份和迁移。 |
| 官网需要基本安全边界 | 无公开注册；管理入口逐请求鉴权；上传、富文本和公开媒体均有约束。   |

首次体验建议依次阅读[快速开始](docs/quick-start.md)、[内容管理指南](docs/content-management.md)和[上线检查清单](docs/deployment.md#上线检查清单)。遇到问题可先查看[故障排查](docs/troubleshooting.md)。
...
- [故障排查](docs/troubleshooting.md)：启动、登录、媒体、磁盘和备份问题。
...
<p align="center">
  <a href="https://xpeach.codes/" target="_blank" rel="noopener noreferrer">
    <img src="docs/assets/sponsors/xpeachcode-platform.png" alt="XPeachCode Platform" width="96">
  </a>
</p>

<p align="center">
  <a href="https://xpeach.codes/" target="_blank" rel="noopener noreferrer"><strong>XPeachCode Platform</strong></a>
</p>
## Operations checklist

For a production deployment, configure a dedicated secret store, back up the SQLite database and media separately, restrict the admin surface, and test restore procedures. Review uploaded files, authentication events, and public metadata before opening the site to external users.
